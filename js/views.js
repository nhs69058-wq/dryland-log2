...
ACTS['tab-history'] = () => { ui.tab = 'history'; ui.calSel = null; render(); window.scrollTo(0, 0); };
...
function sessionDetail(id) {
  ...
  sh.el.addEventListener('click', async (e) => {
    ...
    else if (act === 'del' && (await confirmSheet({ title: '이 기록을 삭제할까요?', body: '지운 기록은 되돌릴 수 없습니다.', ok: '삭제', danger: true }))) {
...
function swimSheet(s) {
  ...
  sh.el.addEventListener('click', async (e) => {
    ...
    if (a.dataset.a === 'del') {
      sh.close();
      if (await confirmSheet({ title: '이 수영 기록을 삭제할까요?', ok: '삭제', danger: true })) {
        S.sessions = S.sessions.filter((x) => x.id !== s.id); save(); render(); toast('삭제했습니다');
      }
      return;
    }
