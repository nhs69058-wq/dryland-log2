...
ACTS.exmenu = (b) => {
  ...
  if (c.kind !== 'routine') opts.push({ label: '지난 기록 · 그래프 보기', fn: () => {
    ...
    ui.progEx = it.ex; ui.tab = 'progress'; render(); window.scrollTo(0, 0);
  } });
...
ACTS.discard = async () => {
  ...
  if (!(await confirmSheet({ title: active ? '이 운동을 취소할까요?' : '이 기록을 삭제할까요?', body: '지운 기록은 되돌릴 수 없습니다.', ok: '삭제', danger: true }))) return;
  if (kp) closeKeypad();
  S.sessions = S.sessions.filter((s) => s.id !== c.obj.id);
  if (active) { S.activeId = null; S.timer = null; wake.off(); }
  save();
  if (ui.edit) removeLayer(editLayer); else render();
  toast('삭제했습니다');
};
